...
const countNumberOfChildren = (arr) => {
  return arr.reduce((count, character) => {
    if (character.children) {
      return count + character.children.length;
    }
    return count;
  }, 0);
};
...
const extractStats = (snorlaxData) => {
  return snorlaxData.stats.reduce((stats, item) => {
    stats[item.stat.name] = item.baseStat;
    return stats;
  }, {});
};
...
const extractChildren = (arr) => {
  return arr
    .filter(character => character.name.toLowerCase().includes('a'))
    .reduce((children, character) => {
      if (character.children) {
